...
exports.table_get = ( async (req, res, next) => {
    ...
    if (req.query.update) {
        ...
        let searchCount = `SELECT COUNT(*) AS COUNT FROM myproducts where name like '%${req.query.search}'`;
        let sql = `SELECT * FROM myproducts where name like '%${req.query.search}%' LIMIT 6 OFFSET ${page}`;
        const query = await Promise.all( [queryDatabase(sql), queryDatabase(searchCount)]);
        if (query[0].length !== 0) {
            return res.status(200).json(query);
        } else {
            return res.status(404).json({err: "Something went wrong."});
